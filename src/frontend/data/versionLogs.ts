...
export const VERSION_LOGS: VersionLog[] = [
  { version: "v0.9.17", date: "2026-06-11", latest: true, items: [
    { b: "📷 카메라 스캔 정확도 개선 — 촬영 후 정밀 인식", t: ": 저해상도 프레임을 계속 인식하던 방식을 버리고, 라벨을 비춘 뒤 '촬영해서 인식'을 누르면 고해상도 한 장을 한 번만 정밀 인식(sparse 모드)합니다. 캡처 이미지 위에 인식된 글자가 #태그로 떠서, 탭해 모델명·품목명·시리얼로 담고 '적용'하면 됩니다. '다시 촬영'으로 재시도." },
    { b: "🖼 첨부 이미지 미리보기", t: ": 일정·메모에 첨부한 이미지가 파일명 칩이 아니라 썸네일로 보입니다(탭하면 원본 열림)." },
  ] },
  { version: "v0.9.16", date: "2026-06-10", items: [
    { b: "📷 실시간 카메라 해시태그 스캔", t: ": 스캔을 열면 사진을 찍는 게 아니라 실시간 카메라 화면 위에 인식된 글자가 #태그로 떠다닙니다. 태그를 탭해 모델명(코드)·품목명·시리얼 중 무엇인지 선택해 담고 '적용'하면 됩니다(카메라 불가 환경은 사진 인식으로 자동 전환)." },
    { b: "일정 '재고 직접 추가'가 반영 안 되던 문제", t: ": 행을 추가하고 별도 '기록' 버튼을 안 누르면 조용히 버려지던 문제를 고쳤습니다. 이제 일정을 저장하면 입력한 행이 자동으로 재고에 기록되고 일정에 연결됩니다." },
  ] },
  { version: "v0.9.15", date: "2026-06-10", items: [
