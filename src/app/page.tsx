...
export default function Home() {
  ...
  const allTxns = useMemo(() => {
    const rows: any[] = [];
    records.filter(r => r.type === 'asset').forEach(r => {
      (r.attrs.txns || []).forEach((tx: any) => rows.push({ ...tx, itemTitle: r.title, itemCode: (r.attrs.code || '').trim() }));
    });
                ...
                >
                  ...
                  {allTxns.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', maxHeight: '60vh', overflowY: 'auto' }}>
                      {allTxns.map((t, i) => (
                        <div key={t.id || i} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 0.6rem', borderRadius: '10px', background: 'var(--surface-elevated)', border: '1px solid var(--surface-elevated-border)' }}>
