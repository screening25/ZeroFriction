...
### 2.5 데이터 영속성 — 하이브리드 저장 모델

Zero-Friction은 **"확정이 필요한 입력은 명시적 저장으로, 가벼운 상호작용은 즉시 자동 영속화"** 하는 하이브리드 모델을 따릅니다. 모든 데이터는 클라이언트 `localStorage`에 로컬 우선(local-first)으로 보관됩니다.

| 저장 방식 | 적용 대상 | 구현 |
| --- | --- | --- |
| **명시적 저장 버튼** | 일정·재고·메모 **등록/수정 모달** | `saveSchedule` · `saveInventory` · `submitMemo` (모달 우측 상단 "저장") |
| **즉시 자동 영속화** | 완료 토글, 환경설정 변경, 활동 로그, 휴지통/복구, 메모 본문 인라인 편집 | mutator 호출 → 즉시 `localStorage` 반영 |

* `src/database/index.ts`의 `saveRecords` / `persistSettings` / `persistActivities`가 영속화 단일 진입점
* `addRecord` / `updateRecord` / `deleteRecord`는 내부에서 `saveRecords()`를 자동 실행
* `handleSettingsChange`는 토큰/색상/밀도 변경 즉시 `persistSettings()` 호출
* `logActivity`는 활동 발생 즉시 `persistActivities()` 호출 (최대 50건 링버퍼 유지)
* 결과: 새로고침·앱 재시작·강제 종료 어떤 시나리오에서도 **데이터 손실 0**
* 백업: 환경설정 → 데이터 내보내기/불러오기로 JSON 단일 파일 교환

> **정확성 노트:** 등록/수정 모달에는 분명한 **"저장" 버튼이 존재**합니다 — 이는 사용자가 입력을 확정하는 의도적 커밋 지점입니다. 자동 영속화는 그 외 가벼운 상호작용(토글·설정·로그 등)에만 적용되어, "매번 눌러야 하는 저장"의 부담만 선택적으로 제거합니다. 즉, *모든* 저장 버튼을 없앤 것이 아니라 *불필요한* 저장 버튼을 없앤 것입니다.

### 2.6 Cmd+F 글로벌 커맨드 팔레트

마우스 없이 키보드만으로 어떤 레코드에도 즉시 도달하는 통합 검색 팔레트입니다. (`src/app/page.tsx`)

| 동작 | 키 | 설명 |
| --- | --- | --- |
| 열기 / 닫기 | **⌘F** / **Ctrl+F** | 브라우저 기본 찾기를 가로채(`preventDefault`) 팔레트 토글, 검색창 자동 포커스 |
| 항목 이동 | **↑ / ↓** | 결과 리스트 내 선택 이동 |
| 선택 | **Enter** | 선택 항목의 상세 모달 즉시 오픈 |
| 닫기 | **Esc** | 팔레트 닫기 |

* **통합 검색 범위:** 일정(제목·설명·카테고리) · 재고(품목명·시리얼·담당자·카테고리) · 메모(제목·본문) — 상위 10건
* **타입별 라우팅:** 선택 시 일정 → 일정 상세, 재고 → 재고 상세, 메모 → 메모 편집 모달로 즉시 진입
* 앱 셸(`ClientLayout`)은 별도로 **⌘K** 커맨드 팔레트도 제공 (전역 명령 실행용)

> **Why:** 탐색 자체가 마찰입니다. 탭 전환·스크롤·필터 클릭을 거치지 않고 **"⌘F → 타이핑 → Enter"** 단 3스텝으로 목적 레코드에 도달하게 해 탐색 비용을 최소화했습니다.
...
| 단축키 | 동작 |
| --- | --- |
| **⌘F / Ctrl+F** | 글로벌 커맨드 팔레트 — 일정·재고·메모 통합 검색 |
| **⌘K / Ctrl+K** | 앱 셸 커맨드 팔레트 |
| **⌘1 / ⌘2 / ⌘3** | 탭 전환 (전체 / 일정 / 재고) |
| **⌘,** | 환경설정 토글 |
| **/** | 상단 커맨드 바 포커스 |
| **Enter** (입력창) | 자연어 분석·등록 실행 |
| **↑ / ↓ / Enter / Esc** | 팔레트 내 이동·선택·닫기 |
...
*문서 작성일: 2026-05-21 · Maintainer: **screening25** · Zero-Friction (Zero-Scheduler) Engineering Master Guide*
